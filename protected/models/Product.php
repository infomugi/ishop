...
class Product extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('code, name, category_id, color, status, stock, brand_id, views, likes, created_id, price, update_id, created_date, update_date, sales, rate, image, callus, sub_category_id', 'required'),
	...
	public function abstrack($category,$title){
		if($title==""){
			return "Jual ".$category . " - ". $title ." Murah Terbaru";
		}else{
			return true;
		}
	}				
	...
	public static function getLastest(){
		$sql = "SELECT p.name, p.price, p.image, p.id_product, p.description, c.name as category FROM product as p LEFT JOIN category as c 
		ON p.category_id=c.id_category  ORDER BY p.created_date DESC LIMIT 9 ";
