...
class PromotionController extends Controller
{
		...
		if(isset($_POST['Promotion']))
		{
			...
			if($model->save()){
				if(strlen(trim($model->image)) > 0) {
					$tmp->saveAs(Yii::getPathOfAlias('webroot').'/image/promotion/big/'.$model->image);
				}				
	...
	public function actionUpdate($id)
	{
		...
		if(isset($_POST['Promotion']))
		{
			$model->attributes=$_POST['Promotion'];
			if($model->status==1){
				Promotion::model()->updateAll(array('status' => 0), 'id_promotion != '.$model->id_promotion.'');
			}else{
				Promotion::model()->updateAll(array('status' => 1), 'id_promotion = '.$model->id_promotion.'');
			}
			if($model->save()){
				$this->redirect(array('view','id'=>$model->id_promotion));
			}
	...
	{
		$model=$this->loadModel($id);
		$imageLocationOrginal = Yii::getPathOfAlias('webroot')."/image/promotion/big/";
